...
import { formatEther, formatUnits, getBigInt } from "ethers";
...
export const MhgdHoardingContractData = () => {
  ...
  const { data: mhgdTokenSymbol } = useScaffoldContractRead({
    contractName: "MhgdToken",
    functionName: "symbol",
  });

  const { data: mhgdTokenBalance } = useScaffoldContractRead({
    contractName: "MhgdToken",
    functionName: "balanceOf",
    args: [address],
  });

  const { data: mhgdTokenAllowance } = useScaffoldContractRead({
    contractName: "MhgdToken",
  ...
  const { data: total_hoarded } = useScaffoldContractRead({
    contractName: "MhgdHoardingContract",
  ...
  return (
    <div className="flex flex-col justify-center items-center bg-[url('/assets/background.jpeg')] bg-[length:100%_100%] py-10 px-5 sm:px-0 lg:py-auto max-w-[100vw] ">
      ...
      >
        ...
        This is the Money Hoarders Global Dollar User Interface
        <div className="flex relative w-40 h-40">
          <Image alt="SE2 logo" className="cursor-pointer" fill src="/mhgd_logo_black.png" />
        </div>
      </div>
      <p>
        {" "}
      ...
      <div className="block justify-between w-full">
        ...
        <div className="bg-accent border border-primary rounded-xl flex">
          ...
          <div className="text-2xl text-right min-w-[3rem] px-2 py-1 flex justify-end font-bai-jamjuree">
          {parseFloat(formatEther(total_hoarded || "0")).toFixed(2)}
          </div>
        </div>
        <div className="bg-accent border border-primary rounded-xl flex">
        ...
        <div>
          <div className="flex justify-between w-full">
            <div className="bg-accent border border-primary rounded-xl flex">
              <div className="p-2 py-1 border-r border-primary flex items-top w-min">Total Minted </div>
              <div className="text-2xl text-right min-w-[3rem] px-2 py-1 flex justify-end font-bai-jamjuree">
              {parseFloat(formatEther(totalMinted || "0")).toFixed(2)}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-3 flex-grow">
        ...
        <div className="grid lg:grid-cols-1 flex-grow">
          <div className="bg-accent border border-primary rounded-xl flex">
            ...
            <div className="text-2xl text-right min-w-[3rem] px-2 py-1 flex justify-end font-bai-jamjuree">
            {parseFloat(formatEther(calculateReward || "0")).toFixed(18)}
            </div>
          </div>
        </div>
      </div>
